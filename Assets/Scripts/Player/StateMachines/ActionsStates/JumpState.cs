...
public class JumpState : TemplateState
{
    ...
    protected override void OnStateEnter(TemplateState previousState)
    {
        ...
        StateMachine.velocity.x -= Mathf.Clamp(_movementParams.jumpForce * movementParams.inertieLoss, 0, StateMachine.velocity.x);
    }
    ...
    protected override void OnStateUpdate()
    {
        ...
        {
            StateMachine.velocity.y = _movementParams.minJump * (1 - _timer / _movementParams.timeToReachJumpHeight);
        }
        else StateMachine.velocity.y = _movementParams.jumpForce * (1 - _timer / _movementParams.timeToReachJumpHeight);
        ...
        {
            if (_movementParams.timeToReachJumpHeight - _timer <= _movementParams.apexModifier* _movementParams.timeToReachJumpHeight * (_movementParams.minJump/ _movementParams.jumpForce))
            {
                Debug.Log("Apex");
